...
# 把下載按鈕放到側邊欄，確保它永遠不會因為報錯而消失
st.sidebar.divider()
csv_data = df.to_csv().encode('utf-8-sig')
st.sidebar.download_button(
    label="📥 下載目前回測數據 CSV",
    data=csv_data,
    file_name=f'txf_backtest_{datetime.now().strftime("%Y%m%d")}.csv',
    mime='text/csv',
)
...
if signal_col not in df.columns or pnl_col not in df.columns:
    st.error(f"🚨 找不到對應欄位 (訊號: {signal_col}, 損益: {pnl_col})。請先在 GitHub 手動執行 update_data.py。")
    st.stop()
