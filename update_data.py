...
def fetch_and_process_data():
    ...
    # 抓取極長資料以確保 120日(600根60K) 運算不會出現 NaN
    ...
    df['Signal_MAD'] = 0
    # 多頭回檔：長線多且 MAD 低點向上；空頭反彈：長線空且 MAD 高點向下
    ...
    for name, sig_col in strat_list:
        ...
        pts = np.where(df[sig_col] == 1, df['Exit_Price'] - df['Entry_Price'], 
              np.where(df[sig_col] == -1, df['Entry_Price'] - df['Exit_Price'], 0))
        # 關鍵：此命名必須與前端一致
        df[f'{name}_PnL_TWD'] = (pts * 0.5 * 50 * df[f'Pos_{name}']) - (100 * df[f'Pos_{name}'])
